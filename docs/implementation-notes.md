...
- Swapped Fernet secrets for Windows DPAPI so SMTP credentials encrypt/decrypt without a shared key (ackend/utils/secret_cipher.py, ackend/services/notifications.py).
- Added NotificationSettings persistence + admin API and extended the scheduling UI with an Email Settings tab (DPAPI-backed password storage) (ackend/services/scheduling/sqlite_database.py, ackend/api/scheduling.py, rontend/src/components/scheduling/NotificationEmailSettingsPanel.tsx).
